...
document.addEventListener('DOMContentLoaded', () => {
  // Load Lottie animations after the initial setup
    const loadingLottie = document.getElementById('loading-lottie');
    if (loadingLottie) {
        loadingLottie.load('https://assets5.lottiefiles.com/packages/lf20_HX0isy.json');
        loadingLottie.play();
    }

    const pptLottie = document.getElementById('ppt-lottie');
    if (pptLottie) {
        pptLottie.load('https://assets9.lottiefiles.com/packages/lf20_p8bfn5to.json');
        // Don't play this one until the overlay is shown
    }
    
     const pdfLottie = document.getElementById('pdf-lottie');
    if (pdfLottie) {
        pdfLottie.load('https://assets9.lottiefiles.com/packages/lf20_p8bfn5to.json');
        // Don't play this one until the overlay is shown
    }
    ...
    const app = {
        ...
        startQuiz: function(questions) {
            ...
            // --- THE FIX ---
            // Show all quiz-related UI elements
         ...
         endQuiz: function() {
            ...
            // --- THE FIX ---
            // Ensure the main container is visible before showing the score screen inside it.
            dom.quizMainContainer.style.display = 'block';

            dom.quizSection.style.display = 'none';
            dom.reviewSection.style.display = 'none'; // This is good practice
        ...
        restartFullQuiz: function() {
            ...
            dom.reviewSection.style.display = 'none'; // Explicitly hide review section
            dom.quizBreadcrumbContainer.style.display = 'none';
            dom.navMenuIcon.style.display = 'none';
            dom.fullscreenBtn.style.display = 'none';
        ...
        bindGlobalEventListeners: function() {
            ...
            dom.typewriterToggle.onchange = () => toggleTypewriterEffect();
