...
export function showFinalScoreScreen() {
    ...
    // Show the main container that holds the score screen
    dom.quizMainContainer.style.display = 'block';

    // Ensure the breadcrumb and global buttons remain visible
    dom.quizBreadcrumbContainer.style.display = 'block';
    dom.navMenuIcon.style.display = 'flex';
    dom.fullscreenBtn.style.display = 'block';

    // Now, manage visibility of sections *inside* the main container
    ...
    if (isLastGroup) {
        // This is the end of the entire quiz
        dom.scoreTitle.textContent = `Quiz Complete! Results for ${cd.groupName}`;
        dom.nextGroupBtn.style.display = 'none';
        dom.restartFullQuizBtn.style.display = 'inline-block';
        dom.shareResultsBtn.style.display = 'inline-block';
    } else {
        // This is the end of an intermediate group
        const nextGroupNum = state.currentGroupIndex + 2;
        dom.scoreTitle.textContent = `Group ${state.currentGroupIndex + 1} Complete!`;
        dom.nextGroupBtn.textContent = `Continue to Group ${nextGroupNum}`;
        dom.nextGroupBtn.style.display = 'inline-block';
        dom.restartFullQuizBtn.style.display = 'none'; // Hide "Restart Full Quiz" to avoid confusion
        dom.shareResultsBtn.style.display = 'none'; // Hide share button until the end
    }
...
function renderScoreVisuals(correct, wrong, timeout, skipped, accuracy, total, attempted) {
    ...
    const correctAndWrongPercent = correctPercent + wrongPercent;
    ...
    const createSegment = (percentage, className) => {
        ...
        setTimeout(() => {
            segment.style.strokeDashoffset = circumference - segmentLength;
        }, 100);

        return segment;
    };

    // Create segments with cumulative percentages for stacking effect
    const skippedSegment = createSegment(100, 'donut-segment-skipped');
    const wrongSegment = createSegment(correctAndWrongPercent, 'donut-segment-wrong');
    const correctSegment = createSegment(correctPercent, 'donut-segment-correct');
    
    // Append in order: bottom layer first (skipped), top layer last (correct)
...
// js/review.js -> inside the startReview function
function startReview() {
    ...
    // --- THE FIX ---
    // Hide the main quiz container (which holds the score screen)
    dom.quizMainContainer.style.display = 'none'; 
    
    // Show the review screen container
...
dom.reviewQuestionNumberEl.innerText = `Reviewing ${state.currentReviewFilter} (${index + 1}/${state.filteredAttempts.length}) | ID: ${attempt.v1_id}`;
